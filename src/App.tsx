...
function App() {
  ...
  const [exported] = useState<Family[]>(
    sessionStorage.getItem("exported") === null
      ? []
      : JSON.parse(sessionStorage.getItem("exported")!)
  );

  const fetchChildren = useCallback(() => {
    setDataset(null);
    axios
      .get("http://localhost:5000/api/children/childrenList")
      .then((t) =>
        setDataset(
          t.data.children.map((chil: Family) => {
            return {
              ...chil,
              exported:
                exported.find((ex) => {
