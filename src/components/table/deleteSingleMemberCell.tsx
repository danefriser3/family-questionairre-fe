...
export const DeleteFamilyMemberCell = ({ params, load }: DialogProps) => {
    ...
    return <DeletionConfirmDialog open={open} handleClose={() => setOpen(false)} handleDeleteAll={() => {
        axios.delete("http://localhost:5000/api/children/deleteById/" + params.id).then(() => {
            const expt = JSON.parse(sessionStorage.getItem("exported")!) as Family[];
            sessionStorage.setItem("exported", JSON.stringify(expt.filter((e: Family) => e.id !== params.id)));
        }).then(() => load());
